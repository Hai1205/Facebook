        ...
        public Response sendMessageWithFiles(String senderId, String conversationId, String content,
                ...
                try {
                        ...
                        if (files != null && files.length > 0) {
                                ...
                                Map<String, Object> uploadResult = cloudinaryService.uploadFile(firstFile);
                                if (uploadResult.containsKey("error")) {
                                        log.error("File upload failed: {}", uploadResult.get("error"));
                                        throw new OurException("Failed to upload file: " + uploadResult.get("error"));
                                }

                                message.setFileName(firstFile.getOriginalFilename());
                                message.setFileSize(firstFile.getSize());
                                message.setMimeType(firstFile.getContentType());
                                message.setFileUrl((String) uploadResult.get("url"));
                                ...
                                if (files.length > 1) {
                                        ...
                                        for (int i = 1; i < files.length; i++) {
                                                if (files[i] != null && !files[i].isEmpty()) {
                                                        Map<String, Object> result = cloudinaryService.uploadFile(files[i]);
                ...
                try {
                        ...
                        if (images != null && images.length > 0) {
                                for (MultipartFile image : images) {
                                        ...
                                        if (uploadResult.containsKey("error")) {
                                                throw new OurException("Failed to upload image: " + uploadResult.get("error"));
                                        }
