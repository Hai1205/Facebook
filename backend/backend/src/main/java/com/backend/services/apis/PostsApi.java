...
import com.backend.utils.mappers.*;
...
public class PostsApi {
                ...
                for (MultipartFile file : files) {
                    if (file != null && !file.isEmpty()) {
                        ...
                        if (uploadResult.containsKey("error")) {
                            log.error("File upload failed for postId: {}", uploadResult.get("error"));
                            throw new OurException("Failed to upload file: " + uploadResult.get("error"));
                        }

                        String mediaUrl = (String) uploadResult.get("url");
                        String mediaPublicId = (String) uploadResult.get("publicId");
                        Post.MediaType mediaType = file.getContentType().startsWith("video") ? Post.MediaType.VIDEO
                                : Post.MediaType.IMAGE;

                        mediaUrls.add(mediaUrl);
                        mediaPublicIds.add(mediaPublicId);
    ...
    public Response createStory(String userId, MultipartFile file, String privacy) {
        ...
        try {
            ...
            if (file != null && !file.isEmpty()) {
                Map<String, Object> uploadResult = cloudinaryService.uploadFile(file);
                if (uploadResult.containsKey("error")) {
                    log.error("File upload failed for story: {}", uploadResult.get("error"));
                    throw new OurException("Failed to upload file: " + uploadResult.get("error"));
                }
                ...
                story.setMediaPublicId(mediaPublicId);
            ...
            if (post.getMediaPublicIds() != null) {
                for (String publicId : post.getMediaPublicIds()) {
                    if (publicId != null && !publicId.isEmpty()) {
                        ...
                        log.debug("Deleted media file for postId={}", postId);
                    }
                }
            }
    ...
    public Response deleteStory(String storyId) {
        ...
        try {
            ...
            if (story.getMediaPublicId() != null && !story.getMediaPublicId().isEmpty()) {
                cloudinaryService.deleteFile(story.getMediaPublicId());
                log.debug("Deleted media file for storyId={}", storyId);
            }
            ...
            if (files != null && !files.isEmpty()) {
                ...
                for (MultipartFile file : files) {
                    if (file != null && !file.isEmpty()) {
                        Map<String, Object> uploadResult = cloudinaryService.uploadFile(file);
                        if (uploadResult.containsKey("error")) {
                            log.error("File upload failed for postId={}: {}", postId, uploadResult.get("error"));
                            throw new OurException("Failed to upload file: " + uploadResult.get("error"));
                        }
